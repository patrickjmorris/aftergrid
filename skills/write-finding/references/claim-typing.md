...
| `causal` | The intervention produced the difference. | Assignment to the groups was randomized, and the Analysis recorded that. |

In v0, randomized assignment is the **only** basis for `causal`. No sample size, effect size, time ordering,
regression, matching, difference-in-differences or set of controls promotes an `associational` Claim. A
design that argues for cause without randomization may still be worth running; its Claim is `associational`
...
randomized assignment, the Claim is `associational` or `descriptive`, whatever the draft sentence sounded
...
3. Were the two sides created by randomized assignment, and does `analysis.yaml` say so — in
...
4. Both sides randomized, same definition version, same window, same timezone? → `causal`.
...
- `causal`: name the randomization the conclusion rests on, and what a broken assignment would mean.
