...
- `executions[].executed_by` is `{ kind, tool, tool_version?, recorded_at }`. `kind: harness` means the Operator's own tool ran the query and `aftergrid record` wrote down the SQL, the parameters and the result; `mode` is then `recorded`, `adapter` and `engine_version` are **absent**, and `input_ids` is **empty** — a recorded execution read the Operator's source, not a retained extract, and carrying either an engine name or a retained input would put a provenance in the digest that nothing observed. The schema applies that rule to `mode: recorded` as well as to `kind: harness`, so the state `/checked-analysis` writes before anything has run (`mode: recorded`, `input_ids: []`, no `executed_by`) is valid and `check` warns that the recording has not happened yet, rather than the whole manifest failing the schema. `kind: adapter` means an aftergrid adapter ran it against the retained inputs and the Engine observed the result — `aftergrid execute` does not write the field today, because `adapter`, `engine_version` and `mode: retained_rerun` already say so, and the value exists so it can start without a schema change. An execution with **no** `executed_by` is an adapter run, and is never read as a recorded one.
...
2. **Population and denominator, at the column.** These are the counter-metric definition's own, so they are checked by binding the number to that definition rather than by comparing prose. A `ref:` value names one column, and the column is where the manifest declares which definition produced it: `results[].columns[].definition_ref` on that column must equal the counter-metric's `{ id, version }`. A `derived:` value has no column of its own, so the binding it can carry is its operands': at least one result behind it must come from an `executions[]` entry whose `definition_refs` include that `{ id, version }`. The definition's SQL fixes the grain, the population and the denominator; the execution's parameters fix the window it ran over; the declared `window` above states that window in the manifest so the two can be compared without reading SQL. A signup count, a text label, or any other column of the same result does not qualify, even when the execution that produced the result pinned the definition.
3. **What the manifest cannot settle: the row.** Population is checked by definition binding at column level; the row's grain is the definition's own grain only when the result's `row_key` is the definition's population key: the reviewer confirms. No field says which population a row key denotes, so a cell in a column correctly declared under the counter-metric can still be one subpopulation of it — `ref:retention_by_platform_arm.mobile_checklist.retained_7d_rate` is `retained_7d`'s own column, and its rows are one platform's signups. `check` accepts it. Whether the row is the population the decision metric was measured over is the method reviewer's to confirm (`skills/analysis-review/references/reviewer-briefs.md`).
4. **Not the coverage window.** `coverage.data_from` / `coverage.data_to` describe what data the Finding holds, which is normally wider than the Question's window (the onboarding exemplar covers signups to 2026-07-12 and their opens to 2026-07-19). The comparison is against `question.window`, which is what the decision metric itself was measured over.
...
| an entry with `ref` whose column declares no `definition_ref` equal to the counter-metric | `counter_metric_missing` at the entry, naming the column |
| a `derived:` entry with no result behind it from an execution pinning that definition | `counter_metric_missing` at the entry |
| two entries for the same counter-metric id | `duplicate_id` at the second entry: one counter-metric, one answer |
