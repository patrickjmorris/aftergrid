...
- `difference`, `ratio` and `percent_change` accept named operands, and a named form is the one to write: their sign depends on which operand is which, both orders are valid arithmetic, and naming the pair is what makes the direction checkable rather than trusted (`docs/contracts/reference-grammar.md`, "Operand direction"). `{ after, baseline }` is a before-and-after comparison; `{ minuend, subtrahend }` on a `difference` and `{ numerator, denominator }` on a `ratio` are the signed values that are not comparisons, and `percent_change` takes `{ after, baseline }` only. A positional pair on one of the three is a `direction_unstated` warning at `manifest.yaml#/derived/<i>`; a named pair on an operation that does not take it, and any named pair on `sum`, `min`, `max` or `percent_of`, is refused with `derived_arity`.
