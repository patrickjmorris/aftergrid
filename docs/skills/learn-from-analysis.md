...
Turns a demonstrated surprise into a compact, retrievable note: what was assumed, what was observed, what a future analyst should check, and when the lesson no longer applies.

Capture without retrieval is a diary. The note must change a later plan or check, and it stays **proposed** until a person promotes it.

## When to reach for it

Type `/learn-from-analysis`, or ask in plain language.

Use it after a verified calculation, a failed approach that taught something durable, or a correction you would be embarrassed to repeat. Do not run it on a generic maxim, a session summary, or a plausible idea with no supporting observation. Use [plan-analysis](plan-analysis.md) on the next question to prove retrieval.
...
An Improve-stage standalone; the Compound Engineering analog in this set. Capture here, retrieve in [plan-analysis](plan-analysis.md) and [analyze](analyze.md). See [Analysis that compounds](../fieldnotes/analysis-that-compounds.md).
