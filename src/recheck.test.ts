...
const cats = (r: ReturnType<typeof check>) => r.errors.map((e) => `${e.category}@${e.location}`);

test("representative fixtures: an automatic policy and a not-automatically-evaluable policy both validate", () => {
  const root = copyInstance();
  for (const f of [NUMERIC, INSUFFICIENT]) {
    const r = check({ dir: join(root, "analytics", "findings", f) });
...
test("missing predicate, method or units fail with schema category and path", () => {
  const root = copyInstance();
  let dir = mutate(root, NUMERIC, (m) => { delete m.claims[0].recheck.predicate; });
  assert.ok(cats(check({ dir })).includes("schema@manifest.yaml#/claims/0/recheck"), cats(check({ dir })).join("\n"));
  dir = mutate(root, NUMERIC, (m) => { delete m.claims[0].recheck.method; });
  assert.ok(cats(check({ dir })).some((c) => c.startsWith("schema@manifest.yaml#/claims/0/recheck")));
  dir = mutate(root, NUMERIC, (m) => { delete m.claims[0].recheck.predicate.unit; });
  assert.ok(cats(check({ dir })).some((c) => c.startsWith("schema@manifest.yaml#/claims/0/recheck/predicate")));
...
test("invalid or unresolved thresholds and evidence references fail with category and path", () => {
  const root = copyInstance();
  let dir = mutate(root, NUMERIC, (m) => { m.claims[0].recheck.predicate.threshold = "ext:no_such_target"; });
  assert.ok(cats(check({ dir })).includes("unresolved_reference@manifest.yaml#/claims/0/recheck/predicate/threshold"), cats(check({ dir })).join("\n"));
  dir = mutate(root, NUMERIC, (m) => { m.claims[0].recheck.predicate.threshold = true; });
  assert.ok(cats(check({ dir })).some((c) => c.startsWith("schema@manifest.yaml#/claims/0/recheck/predicate/threshold")));
  dir = mutate(root, NUMERIC, (m) => { m.claims[0].recheck.evidence[0] = "ref:retention_by_arm.nobody.retained_7d_rate"; });
  assert.ok(cats(check({ dir })).includes("unresolved_reference@manifest.yaml#/claims/0/recheck/evidence/0"));
  dir = mutate(root, NUMERIC, (m) => { m.claims[0].recheck.predicate.operator = "within"; });
  assert.ok(cats(check({ dir })).includes("schema@manifest.yaml#/claims/0/recheck"), "within needs tolerance and baseline");
});

test("non-evaluable policies need a reason and an owner, and are never a boolean", () => {
  const root = copyInstance();
  let dir = mutate(root, NUMERIC, (m) => { delete m.claims[1].recheck.reason; });
  assert.ok(cats(check({ dir })).some((c) => c.startsWith("schema@manifest.yaml#/claims/1/recheck")));
  ...
  assert.ok(cats(check({ dir })).some((c) => c.startsWith("schema@manifest.yaml#/claims/1/recheck")), "no extra boolean field");
});

test("the Question falsifier must reference an executable falsifier Check with matching expected outcome", () => {
  const root = copyInstance();
  let dir = mutate(root, NUMERIC, (m) => { m.question.falsifier.check_id = "no_such_check"; });
  assert.ok(cats(check({ dir })).includes("unresolved_reference@manifest.yaml#/question/falsifier"));
  dir = mutate(root, NUMERIC, (m) => { m.question.falsifier.check_id = "arm_balance"; });
  assert.ok(cats(check({ dir })).includes("schema@manifest.yaml#/question/falsifier"), "must be kind falsifier");
  dir = mutate(root, NUMERIC, (m) => { m.question.falsifier.expected_outcome = "fail"; });
  assert.ok(cats(check({ dir })).includes("schema@manifest.yaml#/question/falsifier"), "expected outcome must agree");
  dir = mutate(root, NUMERIC, (m) => { m.question.falsifier = { kind: "not_evaluable", reason: "none", owner: "x" }; });
  assert.ok(cats(check({ dir })).some((c) => c.startsWith("schema@manifest.yaml#/question")), "a resolved Question cannot carry a not_evaluable falsifier");
});

test("a falsifier business result is distinct from an engine failure; neither runs the evaluator", () => {
  const root = copyInstance();
  let dir = mutate(root, NUMERIC, (m) => { m.checks.find((c: any) => c.id === "falsifier_lift").outcome = "fail"; });
  let c = cats(check({ dir }));
  ...
  const r = check({ dir: join(root, "analytics", "findings", INSUFFICIENT) });
  assert.equal(r.sql_execution, "not_performed");
  assert.ok(r.info.some((i) => /falsifier_cancel_rate=not_run/.test(i)));
});

test("Decision records bind to an existing Finding revision, its digest, its Claim ids and its falsifier; schedules carry a timezone", () => {
  const root = copyInstance();
  ...
  assert.ok(cats(check({ dir })).includes("decision_binding@decisions/dec_2x7v4b9m1kqa.yaml#/rests_on_claims/0"));
  write({ ...good, revisit_when: { ...good.revisit_when, falsifier: { finding_id: good.finding.id, check_id: "arm_balance" } } });
  assert.ok(cats(check({ dir })).includes("decision_binding@decisions/dec_2x7v4b9m1kqa.yaml#/revisit_when/falsifier"));
  write({ ...good, finding: { ...good.finding, revision: 7 } });
  assert.ok(cats(check({ dir })).includes("decision_binding@decisions/dec_2x7v4b9m1kqa.yaml#/finding/revision"));
  write({ ...good, finding: { ...good.finding, content_digest: { algorithm: "sha256", value: "a".repeat(64) } } });
  assert.ok(cats(check({ dir })).includes("decision_binding@decisions/dec_2x7v4b9m1kqa.yaml#/finding/content_digest"));
  write({ ...good, revisit_when: { schedule: { kind: "on_date", date: "2026-11-01" } } });
  assert.ok(cats(check({ dir })).some((c) => c.startsWith("schema@decisions/dec_2x7v4b9m1kqa.yaml#/revisit_when/schedule")), "timezone required");
  write({ ...good, revisit_when: {} });
  assert.ok(cats(check({ dir })).some((c) => c.startsWith("schema@decisions/dec_2x7v4b9m1kqa.yaml#/revisit_when")), "schedule or falsifier required");
  write(good);
  assert.equal(check({ dir }).evidence, "valid");
