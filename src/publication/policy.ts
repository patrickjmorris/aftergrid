...
// proposed Finding cannot widen the allowlist that judges it. The file itself must be guarded by CODEOWNERS or
// branch protection; this module only refuses to trust a policy that is missing, unreadable or self-defeating.
 ...
 * a missing automation identity, or an automation identity that is itself a trusted approver.
 */
export function readPublicationPolicy(start: string, instanceRoot?: string): PolicyResult {
  ...
  if (instanceRoot !== undefined) {
    ...
    let instance: ReturnType<typeof findInstance>;
    try { instance = findInstance(start); }
    catch (e) { return untrusted(resolve(start), `the Instance aftergrid.yaml above this Finding could not be parsed: ${(e as Error).message}`, "fix the YAML; an unreadable policy is never treated as permissive"); }
    if (!instance) return untrusted(resolve(start), "no aftergrid.yaml above this Finding; there is no trusted publication policy", `create the Instance policy file and ${GUARD}`);
    root = instance.root;
