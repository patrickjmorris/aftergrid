...
Whether to keep the onboarding checklist for all new users or remove it. Dana owns the decision. Before the experiment, the team agreed the checklist earns its place if the share of new users who come back within a week is at least {{ext:keep_threshold}} higher with it than without it.
...
- Written for the non-technical product owner profile the team keeps.
- Reviews: none recorded yet. No human has approved this Finding for publication; it is a draft.
...
- Queries: retention_by_arm (primary comparison), retention_by_week_arm (stability by signup week), retention_by_platform_arm (exploratory split), under queries/.
- Results: retention_by_arm, retention_by_week_arm, retention_by_platform_arm, under results/.
- Retained inputs: inputs/users.csv, inputs/events.csv, hashes in the manifest.
