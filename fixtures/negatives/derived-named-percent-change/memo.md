...
- Stated as a percent change against the old onboarding rather than in percentage points, the checklist arm's return rate is higher by {{derived:rate_percent_change}}. The operands of that value are named, so the sign is the one the manifest declares.
